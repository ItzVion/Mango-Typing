...
export const BossGame = () => {
  ...
  if (status === "ready") return <div className="flex min-h-[520px] flex-col items-center justify-center text-center gap-5"><Seo title="Boss Fight Typing Game" description="Defeat the boss by typing words and dodge its attacks." path="/home/games/boss"/><BackButton to="/home/typing-games" label="Back to Games"/><h2 className="text-3xl font-bold">Boss Fight</h2><p className="max-w-md text-sm text-white/45">Type each word to damage the boss. When it attacks, type <b className="text-white">defend</b> before the timer runs out.</p><div className="text-sm text-white/50">{difficulty} · {duration}s</div><button onClick={start} className="rounded-xl border border-white/10 bg-white/[0.06] px-8 py-3 font-semibold hover:bg-white/10 transition">Start</button></div>;
  if (status === "won" || status === "lost") return <div className="flex min-h-[520px] flex-col items-center justify-center gap-5 text-center"><h2 className="text-3xl font-bold">{status === "won" ? "Boss Defeated!" : "You Were Defeated"}</h2><div className="flex gap-8 text-center text-sm"><div><b className="block text-2xl">{wordsTyped}</b><span className="text-white/40">Words</span></div><div><b className="block text-2xl">{dodged}</b><span className="text-white/40">Dodged</span></div><div><b className="block text-2xl">{hitsTaken}</b><span className="text-white/40">Hits</span></div></div><button onClick={start} className="rounded-xl border border-white/10 bg-white/[0.06] px-8 py-3 font-semibold hover:bg-white/10 transition">Try Again</button></div>;
  const words = [word, pool[0], pool[1]].filter(Boolean);
  return <div className="h-[calc(100vh-160px)] min-h-[500px] max-h-[620px] overflow-hidden flex flex-col gap-3" onClick={() => inputRef.current?.focus()}><Seo title="Boss Fight Typing Game" description="Defeat the boss by typing words and dodge its attacks." path="/home/games/boss"/>
    ...
    <div className="relative flex-1 min-h-0 overflow-hidden rounded-2xl border border-white/10 bg-[#101112]" style={{ backgroundImage: "linear-gradient(rgba(255,255,255,.025) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,.025) 1px, transparent 1px)", backgroundSize: "32px 32px" }}>
      ...
      <AnimatePresence>{words.map((w, i) => <motion.div key={`${w}-${i}`} initial={{opacity:0,y:-18}} animate={{opacity:i===0?1:.28,y:0}} className="absolute top-[48%] rounded-lg border border-red-500/70 bg-[#171719] px-4 py-2 text-sm font-medium" style={{left:`${50+(i-1)*17}%`,transform:"translateX(-50%)"}}>{w}</motion.div>)}</AnimatePresence>
      {mode === "defend" && <div className="absolute top-[37%] left-1/2 -translate-x-1/2 w-44 text-center"><div className="text-[11px] font-semibold text-[#F5A623] mb-1">ATTACK — TYPE DEFEND</div><div className="h-1.5 rounded-full bg-white/10 overflow-hidden"><motion.div className="h-full bg-[#F5A623]" animate={{width:`${defendPct}%`}}/></div></div>}
    ...
    <div className="shrink-0 flex gap-2"><input ref={inputRef} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={keyDown} autoFocus placeholder={mode === "defend" ? "Type defend..." : "Type the word..."} className="w-full rounded-xl border border-white/10 bg-white/[0.035] px-4 py-3 text-center text-sm outline-none focus:border-white/20"/><button onClick={submit} className="rounded-xl bg-white px-5 py-3 text-sm font-semibold text-black hover:bg-white/90">Enter</button></div>
  </div>;
};