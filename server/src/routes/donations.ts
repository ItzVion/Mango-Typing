...
router.post("/create-order", optionalAuth, async (req: AuthRequest, res: Response): Promise<any> => {
  ...
  const orderRes = await fetch("https://api.razorpay.com/v1/orders", { method: "POST", headers: { Authorization: `Basic ${auth}`, "Content-Type": "application/json" }, body: JSON.stringify({ amount: raw * 100, currency: "INR", receipt: `vct_${Date.now()}` }) });
...
router.post("/webhook", async (req: Request, res: Response): Promise<any> => {
  ...
  if (!secret || typeof signature !== "string" || !Buffer.isBuffer(req.body)) return res.status(400).json({ error: "Invalid webhook configuration or payload" });
